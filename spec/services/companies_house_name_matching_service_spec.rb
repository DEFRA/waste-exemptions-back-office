...
RSpec.describe CompaniesHouseNameMatchingService, type: :service do
  ...
  describe "#run" do
    ...
    context "when there are no similar companies" do
      before do
        create(:registration, operator_name: "Unique Company A", company_no: "12345678")
        create(:registration, operator_name: "Unique Company B Ltd", company_no: "87654321")
        allow(DefraRubyCompaniesHouse).to receive(:new).and_return(
          instance_double(DefraRubyCompaniesHouse, company_name: nil)
        )
      end

      it "returns an empty hash" do
        expect(service.run).to be_empty
      end
    end
