...
module RenewalReminders
  ...
  RSpec.describe BulkFinalRenewalRemindersTextService do
    describe ".run" do
      ...
      it "sends a final renewal text to active registrations due to expire in 1 week" do
        active_expiring_registration =
          create(
            :registration,
            :with_valid_mobile_phone_number,
            registration_exemptions: [
      ...
      it "sends a final renewal text to active legacy bulk registrations due to expire in 1 week" do
        active_expiring_ncc_registration =
          create(
            :registration,
            :with_valid_mobile_phone_number,
            registration_exemptions: [
        ...
        active_expiring_ncc_registration.update(is_legacy_bulk: true)

        described_class.run

        expect(FinalRenewalReminderTextService).to have_received(:run).with(registration: active_expiring_ncc_registration)
      end

      it "does not send a final renewal text to non-active registrations due to expire in 1 week" do
        expiring_non_active_registration =
          create(
            :registration,
            :with_valid_mobile_phone_number,
            registration_exemptions: [
      ...
      it "does not send a final renewal text to active registrations not due to expire in 1 week" do
        non_expiring_non_active_registration =
          create(
            :registration,
            :with_valid_mobile_phone_number,
            registration_exemptions: [
