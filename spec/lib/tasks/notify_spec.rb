...
RSpec.describe "Notify task", type: :rake do
  ...
  describe "notify:letters:ad_renewals" do
    ...
    it "runs without error" do
      allow(RenewalReminders::BulkNotifyRenewalLettersService).to receive(:run).and_return([build(:registration)])
    ...
    describe "lead time configuration" do
      ...
      context "with the default lead time" do
        it "runs the service with the default lead time" do
          allow(RenewalReminders::BulkNotifyRenewalLettersService).to receive(:run)

          subject.invoke

          expect(RenewalReminders::BulkNotifyRenewalLettersService).to have_received(:run).with(30.days.from_now.to_date)
        end
      end
      ...
      context "with lead time set by environment variable" do
        ...
        it "runs the service with the specified lead time" do
          ...
          expect(RenewalReminders::BulkNotifyRenewalLettersService).to have_received(:run).with(33.days.from_now.to_date)
        end
      end
    end
  end
