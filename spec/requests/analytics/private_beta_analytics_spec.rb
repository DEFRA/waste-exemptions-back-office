...
module Analytics
  RSpec.describe "Private beta analytics" do
    let(:role) { "data_viewer" }
    let(:user) { create(:user, role: role) }

    before { sign_in(user) }

    describe "GET /private_beta_analytics" do
      ...
      context "when user has permission to view analytics" do
        ...
        before do
          get private_beta_analytics_path, params: { start_date: start_date, end_date: end_date }
        end
      ...
      context "when user does not have permission to view analytics" do
        ...
        it "raises an authorization error" do
          get private_beta_analytics_path
