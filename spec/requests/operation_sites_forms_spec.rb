...
RSpec.describe "Operation Sites Forms" do
  let(:transient_registration) { create(:edit_registration, workflow_state: "operation_sites_form", is_multisite_registration: true) }
  ...
  describe "GET operation_sites_form" do
    let(:request_path) { "/#{transient_registration.token}/operation_sites" }

    it "renders the expected template" do
      get request_path
      expect(response).to render_template("operation_sites_forms/new")
    end
