...
module ActionLinksHelper
  ...
  def edit_link_for(resource)
    ...
    Rails.application.routes.url_helpers.new_back_office_edit_form_path(resource.reference)
  end
  ...
  def display_renew_links_for?(resource)
    return false unless WasteExemptionsEngine::FeatureToggle.active?(:enable_renewals)
