...
class CompaniesHouseNameMatchingRunnerService < WasteExemptionsEngine::BaseService
  def self.run_until_done(dry_run: true, report_path: nil)
    loop do
      result = CompaniesHouseNameMatchingBatchService.run_batch(dry_run: dry_run, report_path: report_path)
      ...
      sleep(CompaniesHouseNameMatchingBatchService::TIME_WINDOW)
    end
  end
end
